...
$crud->set_subject("Posts"); //para cambiar el añadir
$crud->set_theme("bootstrap-v4"); //para cambiar el tema
...
$crud->set_subject("Boletines"); //para cambiar el añadir
$crud->set_theme("bootstrap-v4"); //para cambiar el tema
/* $crud -> unset_jquery (); */

$crud->columns('newsletter_tittle', 'newsletter_body');

$crud->callback_before_insert(array($this, 'enviar_mail_callback'));
$crud->callback_before_update(array($this, 'enviar_mail_callback'));


//CAMBIAR NOMBRES DE CAMPOS
$crud->display_as('newsletter_tittle', 'Titulo')
->display_as('newsletter_body', 'Cuerpo de la publicación');

$output = $crud->render();
$this->_example_output($output);
}

function enviar_mail_callback($post_array) {

//enviar correo
$from1 = "carlos.torres_19@hotmail.com";
$from2 = "carlos.torres_19@hotmail.com";
$to = 'carlos.torres_19@hotmail.com';
...
function read_mail_bandejaEntrada($user_id = 38) {
$data['messages'] = $this->Model_mail->read_mail($user_id);

$crud = new grocery_CRUD();
$crud->set_table('mail_holiday');

$crud->columns('mail_date', 'mail_subject');

$crud->order_by('mail_date', 'desc');


$crud->display_as('mail_date', 'Fecha de envio')
->display_as('mail_subject', 'Asunto');



$crud->callback_before_insert(array($this, 'enviar1_mail_callback'));
$crud->callback_before_update(array($this, 'enviar1_mail_callback'));
...
//echo "Hola estamos leyendo el mensaje";
$output = $crud->render();

$this->loadViewRegistro($datos, 'view_bandeja_entrada.php');
// $this->loadViewRegistro($datos, null,$data);
}

function read_mail($user_id = 0) {
$user_id = $this->session->userdata('user_id');
$data['messages'] = $this->Model_mail->read_mail($user_id);
//echo "hola estamos en el backoffice";
...
$crud = new grocery_CRUD();
$crud->set_table('mail_holiday');
$crud->set_subject("Mensaje Nuevo"); //para cambiar el añadir
$crud->set_theme("bootstrap-v4"); //para cambiar el tema
$crud->columns('mail_subject', 'mail_body', 'mail_to', 'mail_date');
$crud->add_fields('mail_from', 'mail_subject', 'mail_body');
$crud->edit_fields('mail_date', 'mail_to', 'mail_subject', 'mail_body');
$crud->order_by('mail_date', 'desc');


$crud->display_as('mail_subject', 'Asunto')
->display_as('mail_body', 'Cuerpo del email')
->display_as('mail_to', 'Enviado por:')
->display_as('mail_from', 'Enviar a:')
->display_as('mail_date', 'Fecha de envio');
...
$this->loadViewRegistro($datos, 'view_message.php');
// $this->loadViewRegistro($datos, null,$data);
}

function read_mail_user($mail_id) {

$data['messages'] = $this->Model_mail->read_mail_user($mail_id);
...
$this->loadViewRegistro($datos, 'view_message.php', $data);
// $this->loadViewRegistro($datos, null,$data);
}

function EditMessageById($mail_id) {

$data['messages'] = $this->Model_mail->EditMessageById($mail_id);
...
function send_mail_user($mail_user_id = 0) {
$mail_user_id = $this->session->userdata('mail_user_id');
...
echo "boton[$aceptar]<br>";
$this->Model_mail->Guardar_datos();

$data['messages'] = $this->Model_mail->SendMail($from1, "HolidayApartment", $to, $subject, $body, $fichero_adjunto = null, $empresa = null);
$data['messages'] = $this->Model_mail->read_mail_user($mail_user_id);
//        $body=null;
...
function vermas_mail($mail_user_id) {
$data['messages'] = $this->Model_mail->vermas_mail($mail_user_id);
$crud = new grocery_CRUD();
$crud->set_table('mail_holiday');
$crud->set_subject("Mensaje Nuevo"); //para cambiar el añadir
$crud->set_theme("bootstrap-v4"); //para cambiar el tema
...
function enviar1_mail_callback($mail_holiday_array) {

//enviar correo
$from1 = "carlos.torres_19@hotmail.com";
$from2 = "carlos.torres_19@hotmail.com";
$to = 'carlos.torres_19@hotmail.com';
$asunto = $mail_holiday_array['mail_subject'];
$cuerpo = $mail_holiday_array['mail_body'];
;
$fichero_adjunto = NULL;

$this->Model_mail->SendMail($from1, $from2, $to, $asunto, $cuerpo, $fichero_adjunto, $empresa = null);

return $mail_holiday_array;
}

function cambiar_pass_user($user_id = 38) {
...
$this->loadViewRegistro($datos, 'cambiar_pass/cuenta.php');
}

public function category() {
$crud = new grocery_CRUD();
$crud->set_table('gnr_post_category');
$crud->set_subject("Categorias"); //para cambiar el añadir
$crud->set_theme("bootstrap-v4"); //para cambiar el tema
...
$crud->set_subject("Etiquetas"); //para cambiar el añadir
$crud->set_theme("bootstrap-v4"); //para cambiar el tema
...
$crud->set_subject("Roles"); //para cambiar el añadir
$crud->set_theme("bootstrap-v4"); //para cambiar el tema
$output = $crud->render();
...
$crud = new grocery_CRUD();
$crud->set_subject("Usuarios"); //para cambiar el añadir
...
$datos['messages'] = $this->Model_mail->read_mail($user_id);
//        echo $user_id;
...
//        echo "estamos en getpropertyactve";
//print_r($datos['properties']);
...
$crud->set_subject("Datos Personales"); //para cambiar el añadir
...
$crud->fields('booking_firstname', 'booking_lastname', 'bookings_check_in', 'bookings_check_out', 'bookings_email', 'bookings_persons', 'bookings_phone', 'bookings_mobile', 'bookings_address', 'bookings_country', 'bookings_city', 'bookings_zip', 'bookings_state');
...
echo "<br>";
echo '<a href="https://holidayapartment.online/backoffice/season">Volver atrás</a>';
return;
}
...
$crud->set_subject("Localización"); //para cambiar el añadir
