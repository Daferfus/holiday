...
class Model_webapp_content extends CI_Model {
    ...
    function LoadHeaderBlog($view) {
        ...
        $num_page = ($this->uri->segment(3)) ? $this->uri->segment(3) : 0;

        $content_header['robots'] = " ";

        $segmento1 = $this->uri->segment(1);
        if ($num_page > 0 || ($segmento1=='cat_blog' || $segmento1 == 'tag'))
            $content_header['robots'] = '<META NAME="ROBOTS" CONTENT="NOINDEX, NOFOLLOW">';
    ...
    public function LoadContent($view, $content, $page_name = null) {
        ...
        if ($view != 'vw_index' && $view != 'vw_search' && $view != 'vw_login' && $view != 'vw_register' && $view != 'vw_register_property' && $view != 'es/vw_sorteo')
            $this->load->view('webapp/' . 'vw_index', $content);
