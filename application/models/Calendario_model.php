...
class Calendario_model extends CI_MODEL {
    ...
    function get_calendar_data($year, $month) {
        ...
        $query = $this->db->select('*')->from('calendars_holiday')->like('fecha', "$year-$month", 'after')->where('property_id',$property_id_active)->get();
        $cal_data = array();
    ...
    public function generar_calendario($year, $month) {
        ...
        $conf_calendar = array(
            'show_next_prev' => TRUE,
            'next_prev_url' => 'https://holidayapartment.online/backoffice/cal', 
    ...
    public function update_calendario($fecha){
        echo " dentro de la función  update_calendario: $fecha";
        $property_id_active = $this->session->property_id;
        //Comprobamos si está reservado 11 , lo ponemos disponible
        $this->db->where('fecha', $fecha);
        $this->db->where('property_id',$property_id_active);
        $query = $this->db->get('calendars_holiday');
        $estado="";
        if ($query->num_rows() == 1) {
             $row = $query->row();
             $estado = $row->estado;
      ...
      //  echo "ESTADO[$estado]<br>";
        ...
        //Actualizamos el dia como reservado poniendo 11.
        $this->db->update('calendars_holiday', $data);
        ...
        if ($query->num_rows() == 0) {
            for ($i = 1; $i <= $dias_total_mes; $i++) {
                ...
                if ($weekDay == 0 || $weekDay == 6) {
                    ...
                    $data[$i] = array('fecha' => $year . "-" . $month . "-" . $i, 'comentario' => '', 'estado' => 'fest','property_id' => $property_id_active);
                }
                //comprobamos si es una fecha anterior a la actual
                else if ($month == date('m') && $i < date('d')) {

                    $data[$i] = array('fecha' => $year . "-" . $month . "-" . $i, 'comentario' => '', 'estado' => 'ant','property_id' => $property_id_active);
                }
                //días posteriores al día actual
                else {

                    $data[$i] = array('fecha' => $year . "-" . $month . "-" . $i, 'comentario' => '', 'estado' => '','property_id' => $property_id_active);
                }

                //insertamos los días del mes en la base de datos
                $this->db->insert('calendars_holiday', $data[$i]);
            }
        }
    }
